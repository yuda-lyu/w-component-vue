...
(function (global, factory) {
  ...
  (global = global || self, global['w-json-view'] = factory());
  ...
  var script = {
    ...
    watch: {
      data: {
        ...
        handler: function handler(value) {
          ...
          var items = vo.parseData(vo.data); //lineNumberWidth, 每個字元使用13px寬度

          var lineNumberWidth = (Math.ceil(Math.pow(size_1(items), 0.1)) + 1) * 13; //save

          vo.items = items;
          vo.lineNumberWidth = lineNumberWidth; //change
    ...
    methods: {
      ...
      getUseItems: function getUseItems() {
        ...
        var vo = this; //items, 直接存取vo.item, 不能用cloneDeep對大數據太耗時
      ...
      parseData: function parseData(d) {
        ...
        function addItem(_ref) {
          var level = _ref.level,
              key = _ref.key,
              keyNumbers = _ref.keyNumbers,
              value = _ref.value,
              valueColor = _ref.valueColor,
              valueComma = _ref.valueComma,
              valueTail = _ref.valueTail,
          ...
          items.push({
            ...
            //bol, 數值後方是否顯示","
            valueTail: valueTail,
            //str, 數值後方結尾符號, 僅用於陣列或物件, 供displayChildren=false時額外顯示出結尾符號
            hasChildren: hasChildren,
            //bol, 是否有子節點, 僅陣列或物件才為true
            displayChildren: vo.styleItem.defDisplayChildren,
            //bol, 是否顯示子節點
            show: true,
            //bol, 是否顯示此節點
        ...
        function pObject(_ref3) {
          var level = _ref3.level,
              key = _ref3.key,
              value = _ref3.value,
              last = _ref3.last;
          var n = size_1(keys_1(value));
          var keyNumbers = {
            type: 'obj',
            n: n
          };
        ...
        function pSelf(_ref4) {
          ...
          if (isarr(value)) {
            ...
            var r = cValue(value);
            var c = vo.styleItem["".concat(r.type, "Color")];
            addItem({
              level: level,
              key: "".concat(key, " :  "),
              value: r.value,
      ...
      toggleItems: function toggleItems(item) {
        //console.log('methods toggleItems',item)
        var vo = this; //toggle

        vo.items[item.index].displayChildren = !vo.items[item.index].displayChildren; //setTimeout
                      ...
                      [
                        _c(
                          "div",
                          {
                            style: [
                              _vm.styleLineNumber,
                              { width: _vm.lineNumberWidth + "px" }
                            ]
                          },
                          [_vm._v(_vm._s(item.lineNumber))]
                        ),
                        _vm._v(" "),
                        _c(
                          "div",
                          {
                            style: [
                              {
                                display: "table-cell",
                                "padding-left": item.paddingLeft + "px"
                              }
                            ]
                          },
                          [
                                                    ...
                                                    [
                                                      _c("g", [
                                                        _c("path", {
                                                          attrs: {
                                                            d:
                                                              "M41.712,415.346c-11.763,0-21.3-9.537-21.3-21.3V21.299C20.412,9.536,29.949,0,41.712,0l346.122,191.697 c0,0,15.975,15.975,0,31.951C371.859,239.622,41.712,415.346,41.712,415.346z"
                                                          }
                                                        })
                                                      ])
                                                    ]
  ...
  __vue_render__._withStripped = true;
    ...
    var WJsonView = normalizeComponent_1(
